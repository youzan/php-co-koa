...
当生成器迭代过程发生未捕获异常，生成器将会被关闭，`Generator::valid`返回false，未捕获异常会从生成器内部被抛向父作用域，嵌套子生成器内部的未捕获异常必须最终被抛向根生成器的calling frame，PHP7中`yield-from`对嵌套子生成器resume时产生的异常，采取`goto try_again传递`+`while`方式层层向上抛出，我们的代码因为递归迭代的原因，未捕获异常需要逆递归栈帧方向层层上抛，性能方便有改进余地。
